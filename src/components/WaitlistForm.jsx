...
const INITIAL = {
  ...
  customerType: '', wasteType: '', wasteQuantity: '',
  frequency: '', referralSource: '',
}
...
const inputCls =
  `w-full px-3.5 py-3 text-sm text-gray-900 bg-gray-50 border border-gray-200
  ...
  if (done) {
    return (
      <div className="text-center py-10 px-2">
        ...
        <h3 className="text-xl sm:text-2xl font-extrabold text-gray-900 mb-3">You're on the list!</h3>
        ...
        <div className="w-full bg-[#f0fdf4] border border-green-200 rounded-xl px-4 py-3.5 mb-5 text-left flex gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-green-800">Check your WhatsApp!</p>
            <p className="text-xs text-green-700 mt-0.5 leading-relaxed">
              We've sent you a welcome message on WhatsApp with more details about TrashGo,
              how the pilot works, and when we'll be in your area.
            </p>
          </div>
        </div>

        <p className="text-sm text-gray-500 leading-relaxed mb-7 max-w-xs mx-auto">
          Our team will contact you personally when the free waste pickup pilot begins in your area.
        </p>
        <a
          ...
          className="inline-flex items-center justify-center gap-2 w-full sm:w-auto
                     bg-[#25d366] text-white font-bold px-7 py-3.5 rounded-xl
                     hover:bg-[#1ebe5d] transition-colors
                     shadow-[0_4px_14px_rgba(37,211,102,0.35)]"
        >
          💬 Join TrashGo WhatsApp Community
        </a>
      </div>
    )
  }
    ...
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4 mt-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Full Name" required>
          <input type="text" value={form.fullName} onChange={set('fullName')}
            placeholder="e.g. Mohamed Kamara" className={inputCls} />
        </Field>
        <Field label="WhatsApp Number" required>
          <input type="tel" value={form.phone} onChange={set('phone')}
            placeholder="+232 76 123456" className={inputCls} />
        </Field>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Location / Area" required>
          <input type="text" value={form.location} onChange={set('location')}
            placeholder="e.g. Freetown, Murray Town" className={inputCls} />
        </Field>
        <Field label="Customer Type" required>
          <select value={form.customerType} onChange={set('customerType')} className={selectCls}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Type of Waste" required>
          <select value={form.wasteType} onChange={set('wasteType')} className={selectCls}>
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-3 mt-0.5">
          {quantityOptions.map(({ label, sub }) => {
            ...
            return (
              ...
              >
                <input type="radio" name="wasteQuantity" value={val}
                  checked={active} onChange={set('wasteQuantity')} className="sr-only" />
      ...
      <Field label="How did you hear about TrashGo?" required>
        <select value={form.referralSource} onChange={set('referralSource')} className={selectCls}>
      ...
      >
        {loading ? (
          <>
            <svg className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin"
                 viewBox="0 0 24 24" />
            Submitting…
          </>
